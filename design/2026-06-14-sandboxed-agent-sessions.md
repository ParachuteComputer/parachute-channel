---
title: "Sandboxed agent sessions + in-page terminal + full-tier deploy (agent)"
description: "Agent's deferred hardening (PLAN.md:153) made real: an isolation envelope around resident Claude sessions, a spawn/scope command graduating launch-session.sh, an in-page xterm.js terminal over the hub WS bridge, and a VPS cloud-init path so the full tier runs on a real host."
---
# Sandboxed agent sessions + in-page terminal + full-tier deploy

**Date:** 2026-06-14
**Status:** Proposed. Scopes agent's PLAN.md Stage-1 "Future: VM/Docker session isolation" line (`PLAN.md:153`) into a concrete build, plus two adjacent deliverables Aaron prioritized on 2026-06-14: an in-page terminal and a real-host deploy path. Not greenlit to build; this settles the shape.
...
Agent **already runs this use case**. `scripts/launch-session.sh` spins up an interactive `claude` process inside a tmux session (`tmux new-session -d -s <name>-agent … exec claude …`), wires it to a named channel over HTTP MCP, and `tmux attach -t <name>-agent` is the documented way to zoom into the live terminal (`scripts/launch-session.sh:130-131`, `:168`). The channel-message transport is bolted on separately as the `/mcp/<channel>` endpoint that pushes the idle-wake `notifications/claude/agent` onto the session (`src/mcp-http.ts:133-155`). So the session is **both** a real terminal (tmux) and a message in/out pipe (MCP), cleanly separable.
...
The system decomposes into three layers, two of which already largely exist in agent/runner/hub. Keeping them named and separate is the architecture.

| Layer | What it bounds | Status today | This doc adds |
|---|---|---|---|
| **Transport / control** | how an operator drives + watches a session | agent UI (`src/admin-ui.ts`, `transports/http-ui.ts`) + hub WS bridge (`ws-bridge.ts`) exist; SSE chat exists, no terminal | the in-page xterm.js terminal over the WS bridge (§5) |
| **Scope** | which vault / channel / MCP a session may touch | the token + strict-MCP machinery exists (agent `src/auth.ts`, runner `spawn.ts` + `mcp-config.ts`) | a ~115-line composer that graduates `launch-session.sh` into a command/API (§4) |
...
**Scope and isolation are orthogonal and both required.** Scope bounds what the session can reach *over the network as an authenticated principal* (vault read/write, which channels). Isolation bounds what the session can reach *as a local process* (the filesystem, other processes, arbitrary network egress, the host control plane). A perfectly-scoped token does nothing to stop `rm -rf ~` or an outbound exfiltration request; a perfect sandbox does nothing to stop a write to a vault the token shouldn't touch. The current launcher has scope (the minted `agent:read agent:write` token, `launch-session.sh:67`) but no isolation. This doc completes the pair.
...
2. **Filesystem: reads and writes both scoped to declared binds.** Writes are confined to the private per-session workspace (today `~/.parachute/agent/sessions/<name>`, `launch-session.sh:37`) plus any `rw` mount the spec declares; *reads* are confined to the runtime/config, the workspace, and declared `ro`/`rw` mounts — **not** the broad host filesystem. This deliberately tightens Anthropic's "reads allowed" default; the reasoning + the `mounts` field are in §4.5.
...
- **Defer parsing untrusted config until after the trust boundary** — already the shape of agent's parse-after-auth patterns. Inbound channel content reaches the session only after the scope/auth gates have run.
...
A terminal can legitimately flood it: a big build log, `yes`, a `cat` of a large file. The hub bridge (`ws-bridge.ts`) is just a blind client↔upstream pipe — it does not expose any flow-control hook for the daemon to read; its only response to overflow is to close both sides. So the flow control must live in **agent's own daemon**, which is itself a Bun WebSocket server and therefore holds the upstream end of the terminal socket as a `ServerWebSocket`. That gives it `ws.getBufferedAmount()` natively — the daemon watches *its own* send-buffer depth and throttles before bytes ever reach the bridge's ceiling:
...
The goal: the agent backend never lets a single terminal's buffered bytes approach 8 MiB, so the hub's blunt cap is a safety net that never fires in normal use.
...
- **Reconnection:** because we attach to tmux (§5.2), a dropped socket just re-attaches — the session never died, and tmux replays scrollback. The reconnect path re-runs the audience gate + agent's operator check.
...
- **The in-page xterm.js terminal** (§5) in agent's UI: WS bridge + `Bun.Terminal` attached to tmux + `surface` audience with operator-gated agent auth + backend flow control + resize/reconnect.
...
- **In-page terminal is IN v1** (not a later option), and lives in **agent's existing UI**, not a separate surface package.
- **Credential = `CLAUDE_CODE_OAUTH_TOKEN` stored as a per-channel secret in agent's credential store**, default one operator token, per-channel override.
...
- **Agent is the home** for this work (session lifecycle + the new isolation); the terminal is an agent-UI concern over the WS bridge.
...
- **R2 — `Bun.Terminal` version floor (low risk).** `Bun.Terminal` (with `resize` / `setRawMode`) is confirmed present in Bun 1.3.14, so the floor is already met for current Bun. Keep a "verify against the pinned Bun at ship time" check (the hub + agent pin must be ≥ that floor), but this is lower-risk than originally stated. Fallback if a pin ever regresses: pty-over-tmux the daemon controls.
...
- **Q3 — does the in-page terminal warrant its own view** in agent's UI, or fold into the existing chat/admin pages? (Decided home is agent's UI; the sub-placement is open.)
...
When Phase 1 firms up, this is an architectural shift (agent moves from "exploration, may retire" to a hardened agent-session gateway with a new deploy tier). Per the workspace architectural-shift discipline, ship a `parachute-patterns/migrations/YYYY-MM-DD-*.md` propagation checklist in the originating PR, and update:

- the workspace `CLAUDE.md` committed-core table row for agent,
...
- agent's own `PLAN.md:153` (mark the isolation step built) + the `CLAUDE.md` Sessions caveat (the "full machine access" line is closed).
